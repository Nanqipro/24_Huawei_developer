...
# class fatigue_driving_detection:  # 本地
    def __init__(self, model_name, model_path):
        ...
        self.height = 1080
        self.fps = 30
        self.frameNum_3s = self.fps * 3        # 3秒对应的帧数
    ...
    def _preprocess(self, data):
        ...
        for k, v in data.items():
            for file_name, bytesIO_object in v.items():
                ...
                try:
                    # 设置 self.capture 为 'run/temp_<file_name>'
                    if not os.path.exists('run'):
                        os.makedirs('run')
                    self.capture = os.path.join('run', f"temp_{file_name}")
    ...
    def _inference(self, data):
        print(data)
        # 初始化结果字典，新增 "faces" 键用于存储人脸坐标
        self.result = {"result": {"duration": 6000, "category": 0, "faces": [], "drowsy": []}}
        self.temp_list = []
        self.temp_action = {"periods": [3, 7], "category": 1}

        # 初始化记录时间戳（起、止时间）的变量
        self.timestamp_start_lookAround = 0
        ...
        if not self.reader.ret:
            return self.result

        self.height, self.width, self.fps, self.video_totalFrameNum = self.reader.get_capInfo()  # 初始化，获取视频信息

        self.frameNum_3s = int(self.fps * 3)  # 3秒的帧数
        self.skip_step = 6  # 固定跳帧间隔

        deal_frame_ps = 3  # 初始化，设置跳帧，每秒处理几帧
        self.skip_step = int(self.fps / float(deal_frame_ps))  # 按照每秒取多少帧来处理，来确定步长
        ...
        self.nowFrameIdx = self.skip_step  # 当前帧（frame）是第几帧

        # 初始化视频写入器用于可视化
        output_video_path = os.path.join('run', f"visualized_{self.file_name}")  # 可视化视频的保存路径
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        # 确保 frameSize 是整数元组
        frame_width = int(self.width * 0.67)
        frame_height = int(self.height)
        self.video_writer = cv2.VideoWriter(output_video_path, fourcc, self.fps, (frame_width, frame_height))  # 注意宽度已被剪裁

        # 打印 output_video_path 以便调试
        print(f"Output video path: {output_video_path}")
        print(f"Frame size for VideoWriter: {(frame_width, frame_height)}")

        # 检查 VideoWriter 是否初始化成功
        if not self.video_writer.isOpened():
            print(f"Error: VideoWriter failed to open with path {output_video_path}")
            return self.result
        ...
        while frame is not None:
            ...
            print(f'start dealing nowFrameIdx : {self.nowFrameIdx} timestamp : {self.getTimestamp()} ms')
            try:
                # 画面剪裁，主驾驶位； [h, w, 通道]
                frame_cropped = frame[:, int(self.width * 0.33): int(self.width), :]
                # 模型1，图像检测
                self.model_1_detect(frame_cropped)
                # 判断该帧中的行为
                self.judge_action()
                # 处理行为计数
                ...
                # 可视化：将带有边界框的帧写入视频
                self.video_writer.write(frame_cropped)
                ...
                if self.failures > 30:  # 失败超过30次就默认返回
                    print('break because failures > 30')
                    break
        ...
        duration = int((infer_end_time - infer_start_time) * 1000)  # 推理持续时间，单位ms
        self.result['result']['duration'] = duration

        # 根据检测到的行为决定category
        self.result['result']['category'] = self.determine_category()

        # 推理结束，添加检测到的面部坐标到结果中
        self.result["result"]["faces"] = copy.deepcopy(self.detected_faces)
        ...
        if len(bbox) > 0:
            for box in bbox:
                ...
                if box[0] == 0:  # 人脸
                    ...
                    # 打印坐标
                    print(f"检测到人脸的坐标: 左上 {face_coords['top_left']}, 右下 {face_coords['bottom_right']}")

                    # 可视化：绘制矩形框
                    cv2.rectangle(frame, face_coords['top_left'], face_coords['bottom_right'], (0, 255, 0), 2)
    ...
    def deal_action_count(self):
        ...
        if self.is_openMouth:
            ...
            if self.openMouth_MoreThan3s:  # 这个行为结束的时候超过3s
                self.timestamp_end_openMouth = nowTimestamp - half_step_timestamp  # 记录 结束时间
                self.temp_action["periods"][0] = self.timestamp_start_openMouth
                self.temp_action["periods"][1] = self.timestamp_end_openMouth
                self.temp_action["category"] = 2
                self.temp_list.append(copy.deepcopy(self.temp_action))
    ...
    def determine_category(self):
        """
        根据检测到的行为列表 self.temp_list 确定视频的最终类别。
        优先级：
            1. 闭眼 (category 1)
            2. 打哈欠 (category 2)
            3. 打电话 (category 3)
        ...
        final_category = 0  # 默认正常驾驶

        for action in self.temp_list:
            cat = action.get("category", 0)
            if cat in priority:
                if final_category == 0 or priority[cat] < priority.get(final_category, float('inf')):
                    final_category = cat
    ...
    def print_result_info(self):
        ...
        # 打印检测到的人脸坐标
        print('检测到的人脸坐标:', self.detected_faces)
...
def run(opt):
    ...
    for filename in filenames:
        ...
        detection.file_name = filename  # **新增**：设置 file_name

        result = detection._inference(1)

        category = result.get('result').get('category', 0)  # 默认类别为0
        infer_duration = result.get('result').get('duration', 6000)  # 默认持续时间为6000ms
        ...
        # 计算推理时长得分
        video_time = round(our_tool.get_video_time(file_path))  # 计算单个视频的视频时长（单位为ms）
        time_score = our_tool.sigmoid(video_time / (infer_duration * opt.time_slow))
    ...
    # 总得分
    # Total_score = F1_score + Total_time_score
    print('\n', opt.test_video)
    # print("Total score", round(Total_score, 3), "F1_score", round(F1_score,4), "Total_time_score", round(Total_time_score,3), '\n')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--right-copy', type=bool,
                        default=False, help='是否将预测正确的视频单独复制到另外的文件夹下')
